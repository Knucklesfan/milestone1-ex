...
Level::init_defaults()
{
  ...
  ia_tiles.resize(height+1,std::vector<unsigned int>());
  bg_tiles.resize(height+1,std::vector<unsigned int>());
  fg_tiles.resize(height+1,std::vector<unsigned int>());
...
Level::load(const std::string& filename)
{
  ...
  vector<int> bg_tm;
  vector<int> fg_tm;
    int emptyheights = 0; //these are the layers that need to be added to ensure that the level fits the screen size.
  ...
  if (strcmp(lisp_symbol(lisp_car(root_obj)), "supertux-level") == 0)
    {
      ...
      if(!reader.read_int("height",  &height)) {
        std::cout << "No height specified for level. Defaulting to 15.\n";
        height = 15;
    ...
    if(height*32 < screen->h) {
      emptyheights = ((screen->h)-height*32)/32 + 1;
      height = screen->h/32 + 1;

    }
    std::cout << emptyheights <<"\n";
            ...
            while (!lisp_nil_p(cur))
              {
                ...
                std::cout << bg_data.y << " before\n";
                reader.read_bool("stay-on-platform", &bg_data.stay_on_platform);
                bg_data.y+=emptyheights*32;
                std::cout << bg_data.y << " after\n";
  ...
  for(vector<int>::iterator it = ia_tm.begin(); it != ia_tm.end(); ++it, ++i)
    {
      if(j >= height)
        {
        std::cerr << "Warning: Level higher than declared height."
