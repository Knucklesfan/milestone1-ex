...
World::draw()
{
  ...
  /* Draw background: */
  for (y = 0; y < level->height; ++y) //levels are mandatorily 15 long???? TODO: fix this TODONE: i think i fixed this
    {
      for (x = 0; x < screen->w/30; ++x)
        {
          if((int)x + (int)(scroll_x / 32) < level->bg_tiles[0].size()) { //fixes a weird bug if the level is too short to cover all tiles.
  ...
  for (y = 0; y < level->height; ++y) //levels are mandatorily 15 long???? TODO: fix this
    {
      for (x = 0; x < screen->w/30; ++x)
        {
          if((int)x + (int)(scroll_x / 32) < level->ia_tiles[0].size()) { //fixes a weird bug if the level is too short to cover all tiles.
            Tile::draw(32*x - fmodf(scroll_x, 32), y * 32,
  ...
  for (y = 0; y < level->height; ++y)
    {
      for (x = 0; x < screen->w/30; ++x)
        {
          if((int)x + (int)(scroll_x / 32) < level->fg_tiles[0].size()) { //fixes a weird bug if the level is too short to cover all tiles.
            Tile::draw(32*x - fmodf(scroll_x, 32), y * 32,
...
void World::scrolling(double frame_ratio)
{
  ...
  if (true || debug_mode)
  {
    ...
    else
    {
      ...
      else if (tux.physic.get_velocity_x() < 0 && scroll_x > tux_pos_x - X_SPACE && true)
        scroll_x = tux_pos_x - X_SPACE;
      else
      {
        if (tux.dir == RIGHT && scroll_x < tux_pos_x - (screen->w - X_SPACE))
            scroll_x = tux_pos_x - (screen->w - X_SPACE);
        else if (tux.dir == LEFT && scroll_x > tux_pos_x - X_SPACE && true)
            scroll_x = tux_pos_x - X_SPACE;
      }
    }
  }
